...
bool ModulePhysics::Start()
{
	...
	bigCircleTexture = App->textures->Load("assets/bigPlanet.png");
	...
	int y = SCREEN_HEIGHT / 2.0f;
	int diameter = SCREEN_WIDTH / 8;
	...
	b2MassData mass = { 1000000000000, {0,0}, 1 };

	big_ball->SetMassData(&mass);
...
update_status ModulePhysics::PreUpdate()
{
	...
	LOG("Number of bodys in world: %d", world->GetBodyCount());
	
#pragma region Newtons_law_of_universal_gravitation

	const float G = 6.67430e-11f; // Gravitational constant

	b2Body* bodyA = world->GetBodyList(); // Get the first body in the list

	while (bodyA != nullptr) {

		if (bodyA->GetType() == b2_dynamicBody) { // Skip if it's a joint or your custom body type

			b2Body* bi = bodyA;
			b2Vec2 pi = bi->GetWorldCenter();
			float mi = bi->GetMass();

			b2Body* bodyB = world->GetBodyList(); // Reset inner loop to the start of the list

			while (bodyB != nullptr) {

				if (bodyB->GetType() == b2_dynamicBody && bodyB != bodyA) { // Skip if it's a joint or the same body
...
#pragma endregion Newtons_law_of_universal_gravitation
...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius)
{
	...
	b2MassData mass = { 1000000, {0,0}, 10 };

	pbody->body->SetMassData(&mass);
...
update_status ModulePhysics::PostUpdate()
{
	...
	// Draw big circle texture

	App->renderer->Blit(bigCircleTexture, METERS_TO_PIXELS(big_ball->GetPosition().x)-50, METERS_TO_PIXELS(big_ball->GetPosition().y)-50, NULL, 1.0f, 0);
